...
import { ArrowRight, ArrowUpRight, Code2, Focus, Gift, Menu, MonitorSmartphone, ShieldCheck, Sparkles, Workflow, X, Zap } from "lucide-react";
import { cn } from "cn";
...
import { studioNavigationProducts, type ProductArtKind, type StudioProduct } from "@/features/studio/product-catalog";
...
function ProductMenuContent() {
  const [selectedSlug, setSelectedSlug] = useState(studioNavigationProducts[0].slug);
  const selectedProduct = studioNavigationProducts.find((product) => product.slug === selectedSlug) ?? studioNavigationProducts[0];
...
function ProductMenuPreview({ product }: { product: StudioProduct }) {
  const Icon = product.availability === "available" ? Sparkles : productIcons[product.artKind];
  const artClass = product.artKind === "code" ? "bg-brand-coral-strong" : product.artKind === "workflow" ? "bg-accent-sage-strong" : "bg-brand-coral";
  const content = <><span className={cn("mb-auto grid min-h-32 place-items-center overflow-hidden rounded-lg text-text-inverted", artClass)} aria-hidden="true"><Icon className="size-12" /></span><Badge variant={product.availability === "available" ? "secondary" : "outline"}>{product.availability === "available" ? "Available now" : "Coming soon"}</Badge><strong className="block font-display text-3xl font-[560] leading-none">{product.name}</strong><small className="block text-sm text-text-secondary">{product.summary}</small>{product.availability === "available" ? <span className="mt-2 inline-flex items-center gap-2 text-[0.8125rem] font-bold text-brand-coral-strong">Explore the product <ArrowUpRight className="size-4" aria-hidden="true" /></span> : <span className="mt-2 text-[0.8125rem] font-bold text-text-subtle">Currently taking shape in the studio.</span>}</>;
  return product.availability === "available" ? <NavigationMenuLink render={<Link href={product.productHref} />} closeOnClick className={teaserClass}>{content}</NavigationMenuLink> : <article className={cn(teaserClass, "hover:bg-accent-gold-soft")} aria-label={`${product.name}, coming soon`}>{content}</article>;
}

function SolutionsMenuContent() {
  return <div className="grid min-h-100 grid-cols-[minmax(0,1fr)_minmax(17rem,0.3fr)] gap-5 p-5"><div className="min-w-0"><div className="mb-4 flex min-h-11 items-center justify-between gap-5 px-3"><p className="text-[0.8125rem] font-bold uppercase tracking-[0.06em] text-text-subtle">How Frameline works</p><NavigationMenuLink render={<Link href="/solutions" />} closeOnClick className="inline-flex min-h-11 items-center gap-2 p-2 text-[0.8125rem] font-bold text-brand-coral-strong">Explore our approach <ArrowRight className="size-4" aria-hidden="true" /></NavigationMenuLink></div><div className="grid grid-cols-3 gap-2">{studioSolutions.map(({ name, description, icon: Icon }) => <div className={cn("flex min-h-36 items-start gap-3 border-b border-divider p-4", menuTextClass)} key={name}><Icon className="mt-1 size-5 text-brand-coral-strong" aria-hidden="true" /><span><strong>{name}</strong><small>{description}</small></span></div>)}</div></div><NavigationMenuLink render={<Link href="/solutions" />} closeOnClick className={cn(teaserClass, "bg-accent-sage-soft hover:bg-accent-sage")}><span className="mb-auto grid min-h-32 place-items-center rounded-lg bg-accent-sage-strong text-text-inverted" aria-hidden="true"><Focus className="size-12" /></span><Badge variant="outline">The idea</Badge><strong className="block font-display text-3xl font-[560] leading-none">Small tools. A short path to making.</strong><small className="block text-sm text-text-secondary">Frameline removes accounts, uploads, and ceremony wherever they are not needed.</small><span className="mt-2 inline-flex items-center gap-2 text-[0.8125rem] font-bold text-brand-coral-strong">See the principles <ArrowUpRight className="size-4" aria-hidden="true" /></span></NavigationMenuLink></div>;
}

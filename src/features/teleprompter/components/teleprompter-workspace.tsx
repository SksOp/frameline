  ...
  return (
    ...
    >
      ...
      <div className="app-stage" data-mobile-view={state.mobileView}>
        <ScriptPane
          text={state.text} draftReady={state.draftReady} persistenceError={state.draftPersistenceError}
          words={derived.words} duration={derived.duration} onClear={clear}
          onTextChange={(text) => dispatch({ type: "textChanged", text })}
        />
        <PreviewPane
          text={state.text} settings={state.settings} timeline={derived.timeline} previewKey={derived.previewKey}
          previewPaused={state.previewPaused} sessionState={session.state}
          onTogglePause={() => dispatch({ type: "previewPauseToggled" })}
          preparedOutput={<PreparedVideo
            videoRef={session.videoRef} sessionState={session.state} loop={state.settings.loop}
            onPlay={session.playPrepared} onPause={session.pausePrepared} onRestart={session.restartPrepared}
            onPlaybackPositionChange={session.syncPlaybackRate}
          />}
        />
      </div>
      {session.error && <div className="app-error" role="alert">{session.error}{session.state === "failed" ? " Try preparing again." : ""}</div>}
      <PreparationStatus state={session.state} progress={session.progress} stale={derived.preparedVideoIsStale} />
      <TeleprompterDock
        mobileView={state.mobileView} sessionState={session.state} preparedVideoIsStale={derived.preparedVideoIsStale}
        action={derived.primaryAction} onMobileViewChange={(view) => dispatch({ type: "mobileViewChanged", view })}
        onOpenSettings={() => dispatch({ type: "settingsOpened" })} onPrimaryAction={primaryAction}
      />
      <TuneDialog

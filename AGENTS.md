...
- Prefer components over reusable CSS. Put static Tailwind utilities and finite variants in the component that owns the presentation. Extract generic repeated behavior into an approved UI primitive or variant, and extract repeated domain structure into a feature component.
- Do not add feature stylesheets, CSS Modules, reusable feature selectors, undiscoverable dynamic class-name fragments (including constructed fragments such as `state-${value}`), or static inline styles. `src/app/globals.css` is reserved for framework/theme imports, shared design tokens, resets, document-wide accessibility behavior, and documented runtime keyframes.
- Inline styles are allowed only for values calculated at runtime or selected by the user, and must remain in the component that owns those values. Next.js `ImageResponse` social artwork is also exempt because it cannot consume the CSS theme; keep its recurring colors in a typed TypeScript palette synchronized with CSS tokens. One-off illustration colors may be literal utilities only when a token would obscure the palette; never repeat a recurring color literal across files.
...
- Keep shared design tokens in `src/app/globals.css`; keep feature-specific presentation in the owning feature components.
